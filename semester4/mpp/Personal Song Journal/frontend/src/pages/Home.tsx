...
const Home = () => {
    ...
    const syncLocalDataWithServer = () => {
        ...
        if (localData && localData.length > 0) {
            localData.forEach((song: ISong) => {
                axios
                    .get(`http://localhost:3000/crud-operations/${song.songId}`)
                    .then((response) => {
                        console.log('syncing with the server.');
                        if (response.data) {
                            axios
                                .put(
                                    `http://localhost:3000/crud-operations/${song.songId}`,
                                    song,
                                )
                                .then((response) => {
    ...
    const onClickDeleteSong = (data: ISong) => {
        ...
        if (indexToDelete) {
            ...
            axios
                .delete(`http://localhost:3000/crud-operations/${data.songId}`)
                .then((response) => {
    ...
    return (
        <div className='container'>
            ...
            <aside className='sidebar'>
                ...
                <button onClick={onGenreInfoClickHandler}>GENRES Info</button>
                <button
