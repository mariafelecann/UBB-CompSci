...
void UI::add_a_tutorial_ui(const Service& service)
{
	...
	if (validator.validate(tutorial)) {
		if (this->service.service_add_tutorial(tutorial) == 1)
		...
		std::cerr << std::endl;
		std::cerr << "error: tutorial argument is invalid!" << std::endl;
		std::cerr << "title, presenter, link must not be empty!" << std::endl;
		std::cerr << "number of minutes and number of likes must be positive!" << std::endl;
		std::cerr << "number of seconds must be between 0 and 59! " << std::endl;
		std::cerr << std::endl;
	}

	std::ofstream fileOut("tutorials.txt");
	if (!fileOut) {
...
void UI::delete_a_tutorial_ui(const Service& service)
{
	...
	if (this->service.service_delete_tutorial(tutorial) == true)
...
void UI::update_a_tutorial_ui(const Service& service)
{
	...
	if (this->service.service_update_tutorial(t) == true)
...
void UI::add_tutorial_to_watch_list(const Service& service, const Service& service_watch_list, const std::string mode)
{
	std::string title, presenter, link;
	int duration_minutes, duration_seconds, number_of_likes;
...
void UI::delete_tutorial_from_watch_list(const Service& service, const Service& service_watch_list, const std::string mode)
{
	std::string title, presenter, link;
	int duration_minutes, duration_seconds, number_of_likes;
...
void UI::print_user_mode_options()
{
	...
	std::cout << "5. open the watchlist in the desired app" << std::endl;
	std::cout << "6. choose another mode" << std::endl;
...
{
	std::cout << "choose the type of file to store your watch list into: CSV / HTML" << std::endl;
	std::string mode;
	std::getline(std::cin, mode);
	while (mode != "CSV" && mode != "HTML")
	{
		if (mode != "CSV" && mode != "HTML")
			std::cout << "oops! invalid choice! try again: " << std::endl;
		std::getline(std::cin, mode);
	}
	...
	while (option != MAXIMUM_OPTION + 1)
	{
		if (option == MINIMUM_OPTION)
		{
			...
			if (option == MINIMUM_OPTION + 1)
				add_tutorial_to_watch_list(service, service_watch_list, mode);
			else
				if (option == MINIMUM_OPTION + 2)
				{
					delete_tutorial_from_watch_list(service, service_watch_list, mode);
				}
				else
					if (option == MINIMUM_OPTION + 3)
					...
					{
						if (option == MINIMUM_OPTION + 4)
							view_watch_list(mode);
						else
							std::cout << "invalid input" << std::endl;
					}
