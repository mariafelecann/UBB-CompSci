...
void UI::add_a_tutorial_ui(const Service& service)
{
	...
	if (this->service.service_add_tutorial(t) == true)
...
{
	DynamicVector<Tutorials> list = this->service.get_tutorials_of_presenter(presenter);
	Tutorials tutorial;
	std::string command;
	if (!presenter.empty())
		for (int i = 0; i < list.get_size_dynamic_vector(); i++)
...
int UI::see_next_tutorial(const Service& service,const Service& service_watch_list, const std::string name, const int& last_time)
{
	...
	DynamicVector<Tutorials> list = this->service.get_all_tutorials();
	Tutorials tutorial;
	if (last_time < list.get_size_dynamic_vector() )
	{
		tutorial = list.get_element(last_time);
		std::cout << " " << tutorial.get_title() << "  " << tutorial.get_presenter() << "  " << tutorial.get_link() << "  " << tutorial.get_minutes() << "  " << tutorial.get_seconds() << "  " << tutorial.get_likes() << std::endl;
		command = "start " + tutorial.get_link();
		std::system(command.c_str());
		return 1;
...
void UI::run_user_console(const Service& service, const Service& service_watch_list)
{
	...
	if (!presenter.empty())
		for (int i = 0; i < list.get_size_dynamic_vector(); i++)
		{
			if (list.get_element(i).get_presenter() == presenter)
			{
				tutorial = list.get_element(i);
